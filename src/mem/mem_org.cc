...
namespace gem5 {
  namespace memory {
    ...
    void init_memOrg(int addr_bus_num_bits, MemOrg * mem_org, bool is_OOP){
      ...
      (CTR_DESIGN == SPLIT128_UNC_DUAL)  || (CTR_DESIGN == SPLIT128_UNC_7bINT_DUAL) ){
        CTR_SIZE=0.5;
        CTRS_PER_MTREE=128;
      }
      ...
      if (is_OOP)
        ...
        (((long long int) 1 ) << addr_bus_num_bits ) / CACHE_LINE * CTR_SIZE;
      mem_org -> MACStoreSize = 
        (((long long int) 1 ) << addr_bus_num_bits ) / CACHE_LINE * MAC_SIZE;
      ...
      for (int i=0; i<mem_org -> numMtreeLevels; i++){
        ...
        MTREE_ARY[mem_org->numMtreeLevels - 1 - i] = 
            (int) (1.0*CACHE_LINE/MTREE_ENTRY_SIZE[mem_org->numMtreeLevels - 1 - i]);
